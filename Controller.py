...
sys.path.append('C:\\Users\\rohan\\AppData\\Local\\Microsoft\\WindowsApps\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\python3.9')
sys.path.append('C:\\Users\\rohan\\AppData\\Local\\Microsoft\\WindowsApps\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\python3')
sys.path.append('C:\\Users\\rohan\\AppData\\Local\\Packages\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\LocalCache\\local-packages\\Python39\\site-packages')
sys.path.append('C:\\Users\\rohan\\AppData\\Local\\Packages\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\LocalCache\\local-packages\\Python39\\dist-packages')
...
def VisionAndPlanner(GoalLocation):
    ...
    #Originally I, but we changed it to depth_data in order to help us better understand
    img_data = cv2.imread(os.path.join(path_dir, 'Frames', 'Frame%04d.png'%(bpy.data.scenes[0].frame_current)))
    #Originally D, but we changed it to depth_data in order to help us better understand 
    depth_data = Exr2Depth(os.path.join(path_dir, 'Depth', 'Depth%04d.exr'%(bpy.data.scenes[0].frame_current)))
    ...
    #Uses Depth value at top left and right corner to determine drone dirction
    #depth_data[1,1] checks for left corner distance
    print(depth_data[1, 1])
    if depth_data[1,1] < 0.1:
        VelX = 0.5
    #depth_data[1, 240] checks for right corner distance
    elif depth_data[1, 240] < 0.1:
        VelX = -0.5
    else:
        #Substracts traffic cylinder location from the camera location
        Camera_Cone_Dist = bpy.data.objects['Traffic Cylinder'].location[0] - bpy.data.objects['Camera'].location[0]
        VelX = math.copysign(0.5, Camera_Cone_Dist)
...
def Controller(TrafficCylinder, Camera):
    ...
    while(not (GoalReached or OutOfBounds or bpy.data.scenes['Scene'].frame_current>=MaxFrames)): 
        Render()   
        VelX, VelY, VelZ= VisionAndPlanner(GoalLocation)
        ...
        Camera.location[1] += VelY # Forward velocity if fixed, m/frame, DO NOT CHANGE!
        ...
        # not working DistToGoal = math.sqrt((GoalLocation[0]-bpy.context.object.location[0])**2 + (GoalLocation[1]-bpy.context.object.location[1])**2 + (GoalLocation[2]-bpy.context.object.location[2])**2)
